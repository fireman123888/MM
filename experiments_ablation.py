...
from utils import generate_pseudo_labels, LabelDrivenContrastiveLoss
...
def train_and_evaluate(mat_path, unlabel_ratio, seed, use_contrastive=True):
    """训练并评估一次实验"""
    ...
    contrastive_loss_fn = LabelDrivenContrastiveLoss(temperature=TEMPERATURE).to(device) if use_contrastive else None
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            if use_contrastive and contrastive_loss_fn is not None:
                ...
                loss_cl = contrastive_loss_fn(global_rep, labels)
                # 也可以对每个视图的编码计算对比损失
                for v in range(model.num_views):
                    loss_cl += contrastive_loss_fn(labeled_outputs['encodings'][v], labels)
            ...
            # 总损失
            loss = loss_x + LAMBDA_U * loss_u + LAMBDA_CL * loss_cl
            loss.backward()
            optimizer.step()
...
def main():
    ...
    for dataset in DATASETS:
        ...
        for unlabel_ratio in UNLABEL_RATIOS:
            accs, f1s = [], []
            for seed in SEEDS:
                acc, f1 = train_and_evaluate(mat_path, unlabel_ratio, seed, use_contrastive=True)
                accs.append(acc)
                f1s.append(f1)
