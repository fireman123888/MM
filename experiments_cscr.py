...
from utils import generate_pseudo_labels, CSCRContrastiveLoss
...
PATIENCE = 20
LAMBDA_U = 1.0
LAMBDA_CSCR = 0.1  # CSCR损失权重
THRESHOLD = 0.95
TEMPERATURE = 0.5


def train_and_evaluate(mat_path, unlabel_ratio, seed, use_cscr=True):
    """训练并评估一次实验"""
    ...
    cscr_loss_fn = CSCRContrastiveLoss(temperature=TEMPERATURE, use_soft_weight=True).to(device) if use_cscr else None
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for batch in train_loader:
            ...
            if use_cscr and cscr_loss_fn is not None:
                # 获取预测概率用于构建软一致性矩阵
                with torch.no_grad():
                ...
                # 对每个视图的编码也计算CSCR损失
                for v in range(model.num_views):
                    view_probs = F.softmax(labeled_outputs['view_logits'][v], dim=1)
                    loss_cscr += cscr_loss_fn(labeled_outputs['encodings'][v], labels, view_probs)
                loss_cscr = loss_cscr / (model.num_views + 1)
            ...
            if any(m.sum().item() > 0 for m in masks):
                ...
                # 对高置信度的无标签数据也计算CSCR损失
                if use_cscr and cscr_loss_fn is not None and masks[-1].sum() > 5:
                    # 只选择高置信度样本
                    high_conf_idx = masks[-1]
                    if high_conf_idx.sum() > 1:
                        high_conf_features = unlabeled_outputs_train['global_representation'][high_conf_idx]
                        high_conf_pseudo = pseudo_targets[-1][high_conf_idx]
                        high_conf_probs = F.softmax(unlabeled_outputs_train['global_logits'][high_conf_idx], dim=1)
                        loss_cscr += cscr_loss_fn(high_conf_features, high_conf_pseudo, high_conf_probs) * 0.5

            # 总损失
            loss = loss_x + LAMBDA_U * loss_u + LAMBDA_CSCR * loss_cscr
            loss.backward()
            optimizer.step()
...
def main():
    print("=" * 70)
    print("MMatch + CSCR类别驱动对比恢复 消融实验")
    print("=" * 70)
    ...
    for dataset in DATASETS:
        ...
        for unlabel_ratio in UNLABEL_RATIOS:
            accs, f1s = [], []
            for seed in SEEDS:
                acc, f1 = train_and_evaluate(mat_path, unlabel_ratio, seed, use_cscr=True)
                accs.append(acc)
                f1s.append(f1)
