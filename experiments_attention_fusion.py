...
def train_and_evaluate(mat_path, unlabel_ratio, seed, use_attention_fusion=True):
    """训练并评估一次实验"""
    ...
    model = MultiViewEncoder(
        ...
        use_attention_fusion=use_attention_fusion  # 启用注意力融合
    ).to(device)
...
def main():
    ...
    for dataset in DATASETS:
        ...
        for unlabel_ratio in UNLABEL_RATIOS:
            accs, f1s = [], []
            for seed in SEEDS:
                acc, f1 = train_and_evaluate(mat_path, unlabel_ratio, seed, use_attention_fusion=True)
                accs.append(acc)
                f1s.append(f1)
